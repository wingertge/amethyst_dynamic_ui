...
use cassowary::{self, strength::*, Constraint, Expression, Variable, WeightedRelation::*};

use super::{Rect, Size};
use crate::{
    layout::{EditVariable, Layout, LayoutId, LayoutVars, VarType},
    Point
};
...
impl LimnSolver {
    ...
    pub fn update_layout(&mut self, layout: &mut Layout) {
        debug!("Updating layout: {}", layout.id);
    ...
    fn add_constraint(&mut self, constraint: Constraint) {
        debug!(
            "adding constraint {}",
            self.layouts.fmt_constraint(&constraint)
        );
    ...
    pub fn hide_layout(&mut self, id: LayoutId) {
        if !self.layouts.layout_hidden(id) {
            ...
            }
            {
                ...
                if layout.hidden_constraints.is_empty() {
                    layout.hidden_constraints.extend(vec![
                        layout.vars.width | EQ(REQUIRED) | 0.0,
                        layout.vars.height | EQ(REQUIRED) | 0.0,
                        layout.vars.left | EQ(REQUIRED) | 0.0,
                        layout.vars.top | EQ(REQUIRED) | 0.0,
                        layout.vars.right | EQ(REQUIRED) | 0.0,
                        layout.vars.bottom | EQ(REQUIRED) | 0.0,
                    ]);
                }
                layout.hidden = true;
            }
    ...
    pub fn debug_constraints(&self) {
        ...
        while let Some(layout) = layouts.pop_front() {
            ...
            println!("EDIT_VARS");
            for edit_var in self.layouts.layouts[&layout].edit_vars.values() {
    ...
    pub fn debug_layout(&self, id: LayoutId) {
        ...
        debug!("{} {}", self.layouts.layout_name(id), bounds);
    }
}
...
impl LayoutManager {
    ...
    pub fn add_constraint(&mut self, constraint: &Constraint) -> bool {
        //log::debug!("Trying to add constraint: {}", self.fmt_constraint(constraint));
        let mut missing_layouts = false;
        for term in &constraint.expr().terms {
            self.constraints
                .entry(term.variable)
                .or_insert_with(HashSet::new)
                .insert(constraint.clone());
            if self.var_ids.contains_key(&term.variable) {
                ...
                //log::debug!("Missing var: {}", self.fmt_variable(term.variable));
    ...
    fn dependent_layouts(&self, constraint: &Constraint) -> Vec<LayoutId> {
        constraint
            ...
            .terms
            .iter()
            .map(|term| self.var_ids[&term.variable])
            .collect()
    }
    ...
    pub fn layout_vars(&self, id: LayoutId) -> Vec<Variable> {
        ...
        layout
            ...
            .array()
            .iter()
            .chain(layout.associated_vars.keys())
            .copied()
            .collect()
    }
    ...
    pub fn fmt_variable(&self, var: Variable) -> String {
        ...
        let layout = &self.layouts[&id];
        let layout_name = layout.name.clone().unwrap_or_else(|| "unknown".to_owned());
    ...
    pub fn fmt_edit_variable(&self, edit_var: &EditVariable) -> String {
        format!(
            "{} {} == {}",
            strength_desc(edit_var.strength),
            self.fmt_variable(edit_var.var),
            edit_var.val
        )
    }
    ...
    pub fn fmt_constraint(&self, constraint: &Constraint) -> String {
        format!(
            "{} {}",
            strength_desc(constraint.strength()),
            self.fmt_expression(&constraint.expr(), constraint.op())
        )
    }
